...
%%
-module(serinets).
-export([acquire/0, acquire/1, acquire/2, create/2, init/2, do/1]).
...
acquire()           -> acquire("localhost", 8088).
acquire(Addr)       -> acquire(Addr       , 443).
acquire(Addr, Port) ->
...
init(Addr, Port) ->
  ...
  case inets:start(httpd, [
    ...
    Failure ->
      p_report(["failed to start inets httpd: ", Failure])
  end.

p_loop(PidHttpd, Addr, Port) ->
  receive
    {report,Info} -> p_report(Info),    p_loop(PidHttpd, Addr, Port);
    restart       -> p_stop(PidHttpd),  init(Addr, Port);
